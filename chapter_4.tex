...
% \begin{figure}[htbt]
%   \centering
%    \includegraphics[height=55mm]{Figure/pipe.eps}
%    \caption{参照画像の例}
%    \label{fig:pipe}
% \end{figure}
...
% \begin{figure}[htbt]
%   \centering
%    \includegraphics[height=55mm]{Figure/pipe_point.eps}
...
\section{結果と考察}
図4には6D姿勢推定の結果の一部が示され、表1には評価指標に基づくパフォーマンス結果がまとめられている。
...
結果として、RGB-D画像ベースの6D姿勢推定の方が精度指標（ADD-suc、Prj-5-suc）においてRGB画像ベースよりも上回った。
特に、形状の特徴が捉えにくい曲管のような物体に対しても、RGB-D画像ベースの方が精度が改善された。
...
% インテル社製の Intel Realsense D435i である RGB-D カメラを
% 使用した。
