...
図\ref{fig:f11}には、Azure Kinect DKの内部パラメータを指定して取得した大規模配管設備のRGB-D画像を示した。
\begin{figure}[htbt]
    \centering
...
図\ref{fig:f12}には、各センサの標準偏差と測定距離の結果をもとにして近似した結果を示した。
\begin{figure}[htbt]
    \centering
...
図\ref{fig:f12}に示したセンサモデルを用いて、Depth画像の取得結果を示した。
...
\begin{table}[h]
    \centering
    \caption{Evaluation of 6D pose estimation}
...
\begin{figure}[htbt]
	\centering
	\includegraphics[height=110mm]{Figure/result_all_sim.eps}
